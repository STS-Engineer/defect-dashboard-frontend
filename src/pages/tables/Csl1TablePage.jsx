...
export default function Csl1TablePage() {
  ...
  async function loadRows() {
    const res = await api.get("/dashboards/workers-csl1");
    setRows(res.data);
  }

  async function handleDelete(row) {
    try {
      await deleteResource("/dashboards/workers-csl1", row.id);
  ...
  return (
    <div style={{ padding: "20px" }}>
      <div style={{ marginBottom: "24px" }}>
        <h1>CSL 1 (Contrôleurs de second niveau)</h1>
      ...
      <GroupedTable
        title="CSL 1 (Contrôleurs de second niveau)"
        keyField="id"
        columns={columns}
        rows={rows}
      ...
      {canAddEdit && showAddForm && (
        <AddElementForm
          title="Ajouter un CSL 1"
          columns={columns}
          groups={uniqueGroups.length > 0 ? uniqueGroups : ["Semaine"]}
          selectedGroup={selectedGroup}
          postEndpoint="/dashboards/workers-csl1"
          onSaved={async () => {

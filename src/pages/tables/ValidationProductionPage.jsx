...
const PROD_VALIDATOR_NAME = "Responsable Production";
...
export default function ValidationProductionPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await api.get("/defects", { params: { status: "ATT_VALIDATION_PROD" } });
  ...
  async function submitDecision() {
    ...
    const payload = {
      prod_validator_name: PROD_VALIDATOR_NAME,
      prod_validation_comment: remarks.trim(),
      treatment_prod_validator_name: PROD_VALIDATOR_NAME,
      treatment_prod_validation_date: now,
      treatment_prod_decision: decision === "validate" ? "Valid\u00e9" : "Refus\u00e9",
      treatment_prod_remarks: remarks.trim(),
      treatment_status: decision === "validate" ? "Att validation qualit\u00e9" : "Refus\u00e9",
    };

    if (decision === "validate") {
      await postWithFallback(
        `/defects/${selected.id}/prod-validation`,
        `/defects/${selected.id}/validate-production`,
        payload
      );
    } else {
      await postWithFallback(
        `/defects/${selected.id}/prod-validation`,
        `/defects/${selected.id}/reject-production`,
        payload
      );
    }
  ...
  return (
    <div className="page-content validation-review-page">
      <div className="page-header-toolbar">
        <div>
          <h1>Validation Production</h1>
          <p className="help-text">Revisez le traitement superviseur avant decision.</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="empty-state">Aucun incident en attente de validation production.</div>
      ) : (
